...
def main2(glb_file_path,pointName='5mm_18_34_56',downsample=10,GSDmm2px=5,bool_alignOnly=False,b='win',bool_generate=False, output_folder_path=None):
    if output_folder_path is None:
        output_folder_path = glb_file_path
    # print('$',pointName)
    bool_confirm=False
    if b=='win' or b =='mac':
        #import pptk
    ...
    try:
        ...
        cv.imwrite(output_folder_path+pointName+'RGB.jpg',cv.cvtColor(grid_RGB,cv.COLOR_RGB2BGR),[int(cv.IMWRITE_JPEG_QUALITY),100])
        # cv.imwrite(glb_file_path+'/Demo/'+pointName+'/'+pointName+'DEM.jpg',grid_map,[int(cv.IMWRITE_JPEG_QUALITY),100])
        print('[Done]',output_folder_path+pointName+'RGB.jpg')
        try:
...
def p2o_main(pointcloud_file_path, output_folder_path):
    ...
    for las_file in las_files:
        ...
        glb_file_path = pointcloud_file_path  # screenshot saving path

        if b == 'win':
            main2(pointName=fileName, glb_file_path=glb_file_path, GSDmm2px=5, bool_alignOnly=0, b=b, bool_generate=0, output_folder_path=output_folder_path)
        else:
